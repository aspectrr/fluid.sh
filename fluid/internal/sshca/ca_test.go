...
func TestEnsureSSHCA(t *testing.T) {
	t.Run("creates CA when files do not exist", func(t *testing.T) {
		tempDir, err := os.MkdirTemp("", "sshca-ensure-test-")
		if err != nil {
			t.Fatalf("failed to create temp dir: %v", err)
		}
		defer func() { _ = os.RemoveAll(tempDir) }()

		keyPath := filepath.Join(tempDir, "ssh-ca", "ssh-ca")
		pubPath := keyPath + ".pub"

		created, err := EnsureSSHCA(keyPath, pubPath, "test-ca")
		if err != nil {
			t.Fatalf("EnsureSSHCA failed: %v", err)
		}
		if !created {
			t.Error("expected created=true when files did not exist")
		}
	...
	t.Run("does not recreate existing CA", func(t *testing.T) {
		tempDir, err := os.MkdirTemp("", "sshca-ensure-test-")
		if err != nil {
			t.Fatalf("failed to create temp dir: %v", err)
		}
		...
		keyPath := filepath.Join(tempDir, "ssh-ca")
		pubPath := keyPath + ".pub"

		// Create CA first
		if err := GenerateCA(keyPath, "test-ca"); err != nil {
			t.Fatalf("failed to generate CA: %v", err)
		}

		// Get original file info
		origInfo, _ := os.Stat(keyPath)
		...
		if created {
			t.Error("expected created=false when files already exist")
		}

		// Verify file was not modified
		newInfo, _ := os.Stat(keyPath)
		if newInfo.ModTime() != origModTime {
			t.Error("existing CA key was modified")
		}
	})

	t.Run("errors on inconsistent state", func(t *testing.T) {
		tempDir, err := os.MkdirTemp("", "sshca-ensure-test-")
		if err != nil {
			t.Fatalf("failed to create temp dir: %v", err)
		}
		...
		// EnsureSSHCA should error due to inconsistent state
		_, err = EnsureSSHCA(keyPath, pubPath, "test-ca")
		if err == nil {
			t.Error("expected error for inconsistent state (only private key exists)")
		}
