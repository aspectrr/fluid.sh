...
`
	...
	assert.Equal(t, "postgresql://test:test@localhost:5432/test", cfg.Database.URL)
	assert.Equal(t, 32, cfg.Database.MaxOpenConns)
...
`
	...
	// Overridden values
	assert.Equal(t, ":3000", cfg.API.Addr)
...
`
	...
	t.Setenv("DATABASE_URL", "postgresql://env:env@localhost/env")
	...
	assert.Equal(t, "postgresql://env:env@localhost/env", cfg.Database.URL)
